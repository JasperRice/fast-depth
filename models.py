...
class Unpool(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.conv_transpose2d(
            x, self.mask.detach().type_as(x).expand(
                num_channels, 1, -1, -1),
...
def conv(in_channels, out_channels, kernel_size):
    ...
    assert 2*padding == kernel_size - \
        1, "parameters incorrect. kernel={}, padding={}".format(kernel_size, padding)
...
def convt_dw(channels, kernel_size):
    ...
    return nn.Sequential(
        nn.ConvTranspose2d(
            channels, channels, kernel_size, stride, padding, output_padding,
...
class Decoder(nn.Module):
    names = ['deconv{}{}'.format(i, dw)
             for i in range(3, 10, 2) for dw in ['', 'dw']]
    names.append("upconv")
    names.append("upproj")
